...
COURSERA_TOKEN = "S7H6OdizEIzODaak"
...
def compute_loss(X, y, w):
    ...
    # TODO:<your code here>
    l = X.shape[0]
    loss = -(np.sum(np.multiply(y, np.log(probability(X,w))) + np.multiply((1-y), (np.log(1-probability(X,w))))))
    loss = loss/l
...
def compute_grad(X, y, w):
    ...
    # TODO<your code here>
    l = X.shape[0]
    yhat = probability(X,w)
    dw = - np.sum((y - yhat) * X.T, axis=1) * 1/l
    return(dw)
...
for i in range(n_iter):
    ...
    G = (alpha * G) + (1 - alpha) * g2 * g2
    w = w - (eta / np.sqrt(G + eps)) * g2
